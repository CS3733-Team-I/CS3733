...
import FoodRequestAPI.utility.request.Language;
...
import static FoodRequestAPI.utility.KioskPermission.NONEMPLOYEE;
import static FoodRequestAPI.utility.KioskPermission.SUPER_USER;
import static FoodRequestAPI.utility.request.RequestType.INTERPRETER;

/**
 * Should handle user credential validation and connection to the user FoodRequestAPI.database for LoginController
...
public class LoginEntity {
    ...
    private LoginEntity(Boolean test){
        ...
        if (test){
            ...
            if (database.getAllEmployees().size() == 0) {
                // if there are no employees in the FoodRequestAPI.database, start as a super user
                currentLogin = new Employee("firstTimeSetup","","","root","",
                        SUPER_USER,RequestType.GENERAL);
            } else {
    ...
    /**
     * Adds every employee from the FoodRequestAPI.database to the logins hashmap
     * @return
     */
    ...
    /**
     * Gets all the login info from the FoodRequestAPI.entity, only allowed if currently a super user.
     * @return
     */
    ...
    /**
     * Creates a list of employee Usernames for all the employees in the FoodRequestAPI.database
     * @return
     */
    ...
    public boolean addUser(String userName, String lastName, String firstName, String password, ArrayList<String> options,
        ...
        // TODO: make reading from the FoodRequestAPI.database more efficient, when someone is adding a lot of users, we don't want this to take forever
        ...
        // limits creating new logins to subordinates in the KioskPermission hierarchy
            ...
            if(userName.length()<=50){
                ...
                //constructs a temporary employee for FoodRequestAPI.database insertion
    ...
    /**
     * a employee cannot be deleted from the FoodRequestAPI.database unless:
    ...
    public boolean updateUsername(String newUsername, String password){
        ...
        //pulling the user from the FoodRequestAPI.database seems cumbersome, but this avoids the problem associated with nonemployees
